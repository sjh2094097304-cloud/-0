 ...
 */
@HiltWorker
class WeatherWidget4x2Worker @AssistedInject constructor(
        ...
        return try {
            ...
            if (firstCity != null) {
                ...
                if (!forceLocationRefresh) {
                    WeatherWidgetUpdater.update4x2All(applicationContext, cached, displayName)
                } else {
                    FileLogger.i(TAG, "doWork: 当前定位仍是默认占位坐标，先渲染定位中状态，避免北京首帧")
                    WeatherWidgetUpdater.update4x2Loading(applicationContext, displayName)
                }
                ...
                if (canRenderWeatherFrame(freshCity)) {
                    WeatherWidgetUpdater.update4x2All(applicationContext, freshWeather, freshName)
                } else {
                    FileLogger.i(TAG, "doWork: 同步后仍无可信当前位置，保持定位中状态")
                    WeatherWidgetUpdater.update4x2Loading(applicationContext, freshName)
                }

            } else {
                FileLogger.w(TAG, "doWork: 无城市数据，渲染空状态")
                WeatherWidgetUpdater.update4x2All(applicationContext, null, null)
            }
    ...
    companion object {
        private const val TAG = "Widget4x2Worker"
